...
CPlayer::CPlayer()
{
	...
	m_pCamera->GenerateProjectionMatrix(1.01f, 5000.0f, 60.0f);//근평면 원평면 
	...
	m_xmf3prePosition = XMFLOAT3(0.0f, 0.0f, 0.0f); //처음값 저장
	m_xmf3prePosition2 = XMFLOAT3(0.0f, 0.0f, 0.0f);
...
void CPlayer::Move(DWORD dwDirection, float fDistance)// dwDirection, 0.4f 
{
	if (dwDirection)
	{
		XMFLOAT3 xmf3Shift = XMFLOAT3(0, 0, 0);
		//dwDirection|=연산을 했으면 움직이는 방향 조절에 비트 연산자를 해서 어떤 움직임을 할지 정해준다.
...
void CPlayer::Move(XMFLOAT3& xmf3Shift, bool bUpdateVelocity)
{
	...
	//update function에서 계속 불리고 있어서 true일때 내가 position을 저장해주지 않아도 된다.
	else
	{
		
		m_xmf3Position = Vector3::Add(xmf3Shift, m_xmf3Position);
		m_pCamera->Move(xmf3Shift);
		//std::cout << *VectorPosition << std::endl;
